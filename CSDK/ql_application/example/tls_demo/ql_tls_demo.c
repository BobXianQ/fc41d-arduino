...
static void tls_demo_ssl_client_main(void *data )
{
...
ql_task_t	tls_thread_handle = NULL;
extern uint32 ql_sta_ip_is_start(void);
void tls_demo_init(void *arg)
{
	...
	if( tls_thread_handle != NULL ) {
		ql_rtos_task_delete(tls_thread_handle);
	}
}


void ql_tls_demo_thread_creat(void)
{
    int ret;
	ret = ql_rtos_task_create(&tls_thread_handle,
	  ...
	  if (ret != kNoErr) {
		os_printf("Error: Failed to create tls test thread: %d\r\n",ret);
		goto init_err;
	  }
