...
#define CUSTOM_FSDATA_INDEX_HTML_FILE_FMT "%s<!DOCTYPE html>\r\n<html><head><meta charset=\"UTF-8\"><title>smart config</title></head><body bgcolor=\"white\" text=\"black\">" \
...
//#if QL_WIFI_AT_SUPPORT
void custom_printf_string_buffer(char *buffer, int len)
{
    if(buffer)
    {
        for (size_t i = 0; i < len; i++)
        {
            bk_printf("%c", buffer[i]);
        }
...
//#endif
	...
	ql_wifi_stainfo_read(&wifi_info);
	ql_uart_baudrate_read(&uart_baud);

#if QL_WIFI_AT_SUPPORT
	extern int ql_uart_baudrate;
	uart_baud = ql_uart_baudrate;
#endif


//#if QL_WIFI_AT_SUPPORT
	file_data_len = strlen((char *)CUSTOM_FSDATA_INDEX_HTML_HEADER) + strlen((char *)CUSTOM_FSDATA_INDEX_HTML_FILE_FMT) + 10 /* for uart band */ + strlen((char *)wifi_info.sta_ssid) + 1 /* \0 */;
//#else
...
//#if QL_WIFI_AT_SUPPORT
	file_data_len = snprintf(file_data, file_data_len - 1, (char *)CUSTOM_FSDATA_INDEX_HTML_FILE_FMT, (char *)CUSTOM_FSDATA_INDEX_HTML_HEADER, (char *)wifi_info.sta_ssid, sta_state_data, uart_baud);
//#else
