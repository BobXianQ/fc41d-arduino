...
private:
        ...
        size_t fillBuffer()
        {
            if(!_buffer){
                ...
                if(!_buffer) {
                    bk_printf("Not enough memory to allocate buffer");
                    _failed = true;
                    return 0;
                }
            }
...
int WiFiClient::connect(IPAddress ip, uint16_t port, int32_t timeout_ms)
{
    ...
    if (res < 0 && errno != EINPROGRESS) {
        bk_printf("connect on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        close(sockfd);
        return 0;
    }
    ...
    if (res < 0) {
        bk_printf("select on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        close(sockfd);
        return 0;
    } else if (res == 0) {
        bk_printf("select returned due to timeout %d ms for fd %d", _timeout, sockfd);
        close(sockfd);
        return 0;
    } else {
        int sockerr;
        ...
        if (res < 0) {
            bk_printf("getsockopt on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
            close(sockfd);
            return 0;
        }

        if (sockerr != 0) {
            bk_printf("socket error on fd %d, errno: %d, \"%s\"", sockfd, sockerr, strerror(sockerr));
            close(sockfd);
            return 0;
        }
    }

#define ROE_WIFICLIENT(x,msg) { if (((x)<0)) { bk_printf("Setsockopt '" msg "'' on fd %d failed. errno: %d, \"%s\"", sockfd, errno, strerror(errno)); return 0; }}
...
int WiFiClient::setSocketOption(int level, int option, const void* value, size_t len)
{
    ...
    if(res < 0) {
        bk_printf("fail on %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
    }
    return res;
}
...
int WiFiClient::getOption(int option, int *value)
{
    ...
    if(res < 0) {
        bk_printf("getOption fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
    }
    return res;
}
    ...
    while(retry) {
        ...
        if(FD_ISSET(socketFileDescriptor, &set)) {
            ...
            if(res > 0) {
                ...
                bk_printf("write fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
                if(errno != EAGAIN) {
...
int WiFiClient::read(uint8_t *buf, size_t size)
{
    ...
    if (_rxBuffer) {
        ...
        if(_rxBuffer->failed()) {
            bk_printf("read fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
            stop();
        }
    }
    return res;
...
int WiFiClient::peek()
{
    ...
    if (_rxBuffer) {
        res = _rxBuffer->peek();
        if(_rxBuffer->failed()) {
            bk_printf("peek fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
            stop();
        }
    }
    return res;
...
int WiFiClient::available()
{
    ...
    if(_rxBuffer->failed()) {
        bk_printf("available fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
        stop();
    }
    return res;
}
    ...
    if (_connected) {
        ...
        if (res <= 0){
          switch (errno) {
              ...
              case ECONNABORTED:
                  _connected = false;
                  bk_printf("Disconnected: RES: %d, ERR: %d", res, errno);
                  break;
              default:
                  bk_printf("Unexpected: RES: %d, ERR: %d", res, errno);
                  _connected = true;
                  break;
          }
        } else {
