...
void ssl_init(sslclient_context *ssl_client)
{
    ...
    mbedtls_ssl_conf_dbg(&ssl_client->ssl_conf, my_debug, 0 );
...
int start_ssl_client(sslclient_context *ssl_client, const IPAddress& ip, uint32_t port, const char* hostname, int timeout, const char *rootCABuff, bool useRootCABundle, const char *cli_cert, const char *cli_key, const char *pskIdent, const char *psKey, bool insecure, const char **alpn_protos)
{
    ...
    if (ssl_client->socket < 0) {
        bk_printf("ERROR opening socket\r\n");
    ...
    if (res < 0 && errno != EINPROGRESS) {
        bk_printf("connect on fd %d, errno: %d, \"%s\"\r\n", ssl_client->socket, errno, strerror(errno));
    ...
    if (res < 0) {
        bk_printf("select on fd %d, errno: %d, \"%s\"\r\n", ssl_client->socket, errno, strerror(errno));
        lwip_close(ssl_client->socket);
        ssl_client->socket = -1;
        return -1;
    } else if (res == 0) {
        bk_printf("select returned due to timeout %d ms for fd %d\r\n", timeout, ssl_client->socket);
        lwip_close(ssl_client->socket);
        ssl_client->socket = -1;
        return -1;
    } else {
        ...
        if (res < 0) {
            bk_printf("getsockopt on fd %d, errno: %d, \"%s\"\r\n", ssl_client->socket, errno, strerror(errno));
            lwip_close(ssl_client->socket);
            ssl_client->socket = -1;
            return -1;
        }
        ...
        if (sockerr != 0) {
            bk_printf("socket error on fd %d, errno: %d, \"%s\"\r\n", ssl_client->socket, sockerr, strerror(sockerr));
            lwip_close(ssl_client->socket);
            ssl_client->socket = -1;
            return -1;
        }
...
#define ROE(x,msg) { if (((x)<0)) { log_e("LWIP Socket config of " msg " failed."); return -1; }}
    ...
    if (1 || insecure) {
        ...
        ret = arduino_crt_bundle_attach(&ssl_client->ssl_conf);
    ...
    bk_printf("Setting hostname for TLS session...\r\n");
    ...
    while ((ret = mbedtls_ssl_handshake(&ssl_client->ssl_ctx)) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            bk_printf("SSL/TLS handshake err...\r\n");
            return handle_error(ret);
        }
        if((millis()-handshake_start_time)>ssl_client->handshake_timeout)
        {
            bk_printf("mbedtls_ssl_handshake timeout...\r\n");
            return -1;
        }
    ...
    bk_printf("Verifying peer X.509 certificate...\r\n");

    if ((flags = mbedtls_ssl_get_verify_result(&ssl_client->ssl_ctx)) != 0) {
        memset(buf, 0, sizeof(buf));
        mbedtls_x509_crt_verify_info(buf, sizeof(buf), "  ! ", flags);
        bk_printf("Failed to verify peer certificate! verification info: %s\r\n", buf);
        return handle_error(ret);
    } else {
        bk_printf("Certificate verified.\r\n");
    }
