...
void ql_uart_demo_thread(void *param)
{
	...
	recv_buf = os_malloc(uart_len * sizeof(UINT8));
	send_buf = os_malloc(uart_len * sizeof(UINT8));
	...
	os_memset(recv_buf,0,uart_len);
	os_memset(send_buf,0,uart_len);
	...
	ret = ql_uart_set_tx_cb(UART_TEST_PORT, uart_tx_test_callback,NULL);
	if (ret != QL_UART_SUCCESS)
	{
		ql_uart_log("ql_uart_set_tx_cb failed\r\n");
		goto uart_test_exit;
	}
	ql_uart_set_rx_cb(UART_TEST_PORT, uart_rx_test_callback,NULL);
	...
	ql_uart_log("uart rx test,please enter %d bytes using uart%d \r\n",uart_len,UART_TEST_PORT+1);
	...
	while(1)
	{
	  ...
	  if(ql_uart_read(UART_TEST_PORT, recv_buf, uart_len) == 0)
	    {
			break;
		}
		ql_rtos_task_sleep_ms(10);
	}
	ql_uart_log("uart data_len:%d\r\n",uart_len);
	for(i=0; i<uart_len; i++)
	{
